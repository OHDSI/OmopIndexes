...
To illustrate the functionality of OmopIndices, we will use the
...
Let's create a *sinusitis* cohort to use as a mock example.
...
cohort |>
  select(person_id, cohort_start_date, charlson, efi, hfrs, bmi,
...
cohort |>
  select(person_id, cohort_start_date, hfrs, hfrs_categories) |>
  glimpse()
```
